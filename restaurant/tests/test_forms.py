...
class TestBookingForm(TestCase):
    ...
    def test_clean_place(self):
        ...
        self.assertEqual(e_1.exception.message, f'За данным столиком всего {table.places} мест. Выберете столик с большим количеством мест или позже забронируйте соседний столик')

        with self.assertRaises(ValidationError) as e_2:
            form_incorrect_too.clean_place()
        self.assertEqual(e_2.exception.message, f'Должно быть занято хотя бы одно место за столиком')
    ...
    def test_clean_table_active_table(self):
        ...
        booking_active = Booking.objects.create(user=user, table=table, places=4, description='test', notification=0,
                                              date_field=date_next, time_start=time_next_1h,time_end=time_next_2h, active=True)
        booking_active.save()

        self.assertTrue(Booking.objects.all().count() > 0)
        time_border = timezone.now() - timezone.timedelta(minutes=45)

        correct_data = {'date_field': date_next, 'time_start': datetime.time(14, 50, 0), 'time_end': datetime.time(16, 50, 0), 'table': table}
        incorrect_data = {'date_field': date_next, 'time_start': datetime.time(8, 50, 0) , 'time_end': datetime.time(12, 20, 0), 'table': table}
